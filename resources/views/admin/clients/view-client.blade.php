                        ...
                        <div class="card">
                          ...
                          <div class="card-body">
                            ...
                            @foreach($clients as $row)
                                <tr>
                                  <td>{{ $loop->index+1 }}</td>
                                  <td><a href="{{ asset('images/backend_images/client/'.$row->image) }}"><img src="{{ asset('images/backend_images/client/'.$row->image) }}" width="100px"></a></td>                                         
                                  <td>{!! Str::limit($row->title,20)!!}</td> 
                                  <td>{{date('d F Y', strtotime($row->updated_at)) }}</td>
                                  <td>
                                    <a class="btn btn-default"  id="editAbout" href="{{ url('/admin/edit-client/'.$row->id) }}" title="Edit About"><i class="fa fa-edit" style="color: #000;"></i></a> &nbsp;&nbsp;
                                    <a class="btn btn-danger" onclick="return confirm('Are you sure?')" href="{{ url('/admin/delete-client/'.$row->id) }}"><i class="fa fa-trash"></i></a> &nbsp;
                                  </td>
                                </tr>
